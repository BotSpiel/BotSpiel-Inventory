...
namespace BotSpiel.DataAccess.Data
{
    ...
    public class PickBatchPickingDB : DbContext
    {

        public PickBatchPickingDB(DbContextOptions<PickBatchPickingDB> options)
            : base(options)
        {
        }
...
*/
		...
		public DbSet<PickBatchPicking> PickBatchPicking { get; set; }
		public DbSet<PickBatchPickingPost> PickBatchPickingPost { get; set; }
        ...
        {
            modelBuilder.Entity<PickBatchPicking>()
                .ToTable("tx_vw_PickBatchPicking")
                .HasKey(c => new { c.ixPickBatchPick });
            modelBuilder.Entity<PickBatchPickingPost>()
                .ToTable("tx_vw_PickBatchPickingPost")
                .HasKey(c => new { c.ixPickBatchPick });
        ...
        {
            var changes = 0;
            foreach (var e in ChangeTracker.Entries().Where(e => (e.State != EntityState.Unchanged) && (e.State != EntityState.Detached) && (e.Entity is PickBatchPickingPost)).ToList())
            {
                var tx_vw_pickbatchpickingpost = e.Entity as PickBatchPickingPost;
                switch (e.State)
                {
                    case EntityState.Added:
                        ...
                        {
                            con.Open();
                            var sPickBatchPick = cmd.CreateParameter();
                            sPickBatchPick.ParameterName = "p0";
                            sPickBatchPick.Value = tx_vw_pickbatchpickingpost.sPickBatchPick;
                            var ixPickBatch = cmd.CreateParameter();
                            ixPickBatch.ParameterName = "p1";
                            ixPickBatch.Value = tx_vw_pickbatchpickingpost.ixPickBatch;
                            var ixInventoryUnit = cmd.CreateParameter();
                            ixInventoryUnit.ParameterName = "p2";
                            ixInventoryUnit.Value = tx_vw_pickbatchpickingpost.ixInventoryUnit;
                            var nBaseUnitQuantityPicked = cmd.CreateParameter();
                            nBaseUnitQuantityPicked.ParameterName = "p3";
                            nBaseUnitQuantityPicked.Value = tx_vw_pickbatchpickingpost.nBaseUnitQuantityPicked;
                            var sPackToHandlingUnit = cmd.CreateParameter();
                            sPackToHandlingUnit.ParameterName = "p4";
                            sPackToHandlingUnit.Value = tx_vw_pickbatchpickingpost.sPackToHandlingUnit;
                            var ixHandlingUnit = cmd.CreateParameter();
                            ixHandlingUnit.ParameterName = "p5";
                            ixHandlingUnit.Value = tx_vw_pickbatchpickingpost.ixHandlingUnit;
                            var UserName = cmd.CreateParameter();
                            UserName.ParameterName = "p6";
                            UserName.Value = tx_vw_pickbatchpickingpost.UserName;

                            var ixPickBatchPick = cmd.CreateParameter();
                            ixPickBatchPick.ParameterName = "p7";
                            ixPickBatchPick.DbType = DbType.Int64;
                            ixPickBatchPick.Direction = ParameterDirection.Output;

                            var sql = new StringBuilder();
                            sql.Append(@"exec dbo.tx_sp_CreatePickBatchPicking ");
                            sql.Append("@sPickBatchPick = @p0, ");
                            sql.Append("@ixPickBatch = @p1, ");
                            sql.Append("@ixInventoryUnit = @p2, ");
                            sql.Append("@nBaseUnitQuantityPicked = @p3, ");
                            sql.Append("@sPackToHandlingUnit = @p4, ");
                            sql.Append("@ixHandlingUnit = @p5, ");
                            if (tx_vw_pickbatchpickingpost.UserName != null) { sql.Append("@UserName = @p6, "); }  
                            sql.Append("@ixPickBatchPick = @p7 output "); 
                            cmd.CommandText = sql.ToString();

                            cmd.Parameters.Add(sPickBatchPick);
                            cmd.Parameters.Add(ixPickBatch);
                            cmd.Parameters.Add(ixInventoryUnit);
                            cmd.Parameters.Add(nBaseUnitQuantityPicked);
                            cmd.Parameters.Add(sPackToHandlingUnit);
                            cmd.Parameters.Add(ixHandlingUnit);
                            if (tx_vw_pickbatchpickingpost.UserName != null) { cmd.Parameters.Add(UserName); }

                            cmd.Parameters.Add(ixPickBatchPick); 
                            cmd.ExecuteNonQuery();
                            tx_vw_pickbatchpickingpost.ixPickBatchPick = (Int64)ixPickBatchPick.Value;
                            con.Close();
                        }
						e.GetInfrastructure().MarkAsTemporary(e.Metadata.FindProperty("ixPickBatchPick"), false);
						e.State = EntityState.Detached;
                        break;

                    case EntityState.Modified:
                        Database.ExecuteSqlCommand("exec dbo.tx_sp_ChangePickBatchPicking @ixPickBatchPick = @p0, @sPickBatchPick = @p1, @ixPickBatch = @p2, @ixInventoryUnit = @p3, @nBaseUnitQuantityPicked = @p4, @sPackToHandlingUnit = @p5, @ixHandlingUnit = @p6, @UserName = @p7", tx_vw_pickbatchpickingpost.ixPickBatchPick, tx_vw_pickbatchpickingpost.sPickBatchPick, tx_vw_pickbatchpickingpost.ixPickBatch, tx_vw_pickbatchpickingpost.ixInventoryUnit, tx_vw_pickbatchpickingpost.nBaseUnitQuantityPicked, tx_vw_pickbatchpickingpost.sPackToHandlingUnit, tx_vw_pickbatchpickingpost.ixHandlingUnit, tx_vw_pickbatchpickingpost.UserName);
                        e.State = EntityState.Detached;                            
						break;

                    case EntityState.Deleted:
                        Database.ExecuteSqlCommand("exec dbo.tx_sp_DeletePickBatchPicking @ixPickBatchPick = @p0, @sPickBatchPick = @p1, @ixPickBatch = @p2, @ixInventoryUnit = @p3, @nBaseUnitQuantityPicked = @p4, @sPackToHandlingUnit = @p5, @ixHandlingUnit = @p6, @UserName = @p7", tx_vw_pickbatchpickingpost.ixPickBatchPick, tx_vw_pickbatchpickingpost.sPickBatchPick, tx_vw_pickbatchpickingpost.ixPickBatch, tx_vw_pickbatchpickingpost.ixInventoryUnit, tx_vw_pickbatchpickingpost.nBaseUnitQuantityPicked, tx_vw_pickbatchpickingpost.sPackToHandlingUnit, tx_vw_pickbatchpickingpost.ixHandlingUnit, tx_vw_pickbatchpickingpost.UserName);
                        e.State = EntityState.Detached;                           
						break;
                }
